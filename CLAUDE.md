...
แพลตฟอร์ม SaaS ให้ร้านค้าไทยขนาดเล็ก (เสื้อผ้า / แฟชั่น / ของเล่นเด็ก) **เช่าใช้รายปี** เพื่อเปิดร้านออนไลน์ของตัวเองบน subdomain (เช่น `baannoi.shopdashth.com`) หรือ custom domain ของร้านเอง

ระบบมี 3 ผิวสัมผัส (surface):

| Surface | ผู้ใช้ | URL pattern | Auth |
|---|---|---|---|
| **Storefront** | ลูกค้าปลายทาง | `{slug}.shopdashth.com` หรือ custom domain | ไม่ต้อง login (guest checkout) |
| **Store Admin** | เจ้าของร้าน (tenant) | `{slug}.shopdashth.com/admin` | Supabase Auth (email+password) role=`store_owner`/`store_staff` |
| **Super Admin** | เจ้าของแพลตฟอร์ม | `admin.shopdashth.com` | Supabase Auth role=`super_admin` |
...
| Deploy | Vercel (แนะนำ) หรือเทียบเท่าที่รองรับ wildcard domain | ต้องรองรับ `*.shopdashth.com` |
...
```
admin.shopdashth.com/*          → app/(super-admin)/*
{slug}.shopdashth.com/admin/*   → app/(store-admin)/*   (แนบ x-tenant-slug: {slug})
{slug}.shopdashth.com/*         → app/(storefront)/*    (แนบ x-tenant-slug: {slug})
...
if (host === 'admin.shopdashth.com') return rewrite('/super-admin' + path);

let slug: string | null = null;
if (host.endsWith('.shopdashth.com')) {
  slug = host.replace('.shopdashth.com', '');
} else {
...
```
[ลูกค้าเปิด shopdashth.com/signup]
...
[POST /api/signup — service role, transaction เดียว]
  ...
  ▼
[redirect → https://{slug}.shopdashth.com/admin — ใช้งานได้ทันที (trial)]
...
Subdomain ไม่ต้อง provision อะไรเพิ่ม: wildcard `*.shopdashth.com` ชี้เข้าแอปอยู่แล้ว middleware resolve slug ได้ทันทีที่แถว tenants ถูก insert
...
| ลำดับ | งาน | ไฟล์หลัก |
|---|---|---|
| 3.1 | Layout Super Admin ที่ `admin.shopdashth.com` + guard role=super_admin + seed super admin user | `app/super-admin/layout.tsx` |
...
- ข้อความ error ต้องบอก "ค่าที่พบจริง vs ค่าที่ต้องเป็น" เช่น: "พบ CNAME ชี้ไป `old-host.com` — ต้องแก้เป็น `cname.shopdashth.com`" + เตือนเรื่อง DNS propagation (อาจรอถึง 24 ชม.) + ลิงก์คู่มือของ registrar ยอดนิยม (Godaddy/Namecheap/Cloudflare) เป็นภาษาไทย
- ระหว่างยังไม่ `active`: subdomain `.shopdashth.com` ใช้งานได้ปกติเสมอ (custom domain เป็น "เพิ่ม" ไม่ใช่ "แทนที่")
...
```
app/
  (public)/            # shopdashth.com: landing, pricing, signup
  storefront/          # rewrite เป้าหมายของ {slug}.shopdashth.com
  store-admin/         # rewrite เป้าหมายของ {slug}.shopdashth.com/admin
  super-admin/         # rewrite เป้าหมายของ admin.shopdashth.com
...
ROOT_DOMAIN=shopdashth.com              CRON_SECRET=
