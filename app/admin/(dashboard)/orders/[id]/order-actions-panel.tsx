...
export function OrderActionsPanel({ orderId, status }: { orderId: string; status: OrderStatus }) {
  ...
  return (
    <div className="space-y-3 rounded-md border border-gray-200 bg-white p-4">
      ...
      {status === 'packing' && (
        <form action={shipAction} className="space-y-2">
          <div className="flex flex-wrap gap-2">
            <select name="carrier" required defaultValue="" className="rounded-md border border-gray-300 px-3 py-2 text-sm">
      ...
      {status === 'cancelled' && <p className="text-sm text-red-600">ออร์เดอร์นี้ถูกยกเลิกแล้ว</p>}
