...
// hero แนวตั้งโทนฟ้า (1200x1000) สำหรับธีม toys-01 (hero 'split-panel' — รูปฝั่งขวาของแผงฟ้า)
// พื้นหลัง #dcedfb = token --color-secondary ของ preset toys-01 เพื่อให้กลืนกับแผง
function toysHero2Svg() {
  const SKY_PANEL = '#dcedfb';
  return `<svg width="1200" height="1000" xmlns="http://www.w3.org/2000/svg">
  <rect width="1200" height="1000" fill="${SKY_PANEL}"/>
  <ellipse cx="330" cy="180" rx="110" ry="42" fill="#fff" opacity="0.8"/>
  <ellipse cx="950" cy="130" rx="140" ry="50" fill="#fff" opacity="0.65"/>
  <ellipse cx="620" cy="880" rx="430" ry="34" fill="${TP.ink}" opacity="0.10"/>
  <path d="M180 420 l8 16 17 2.4 -12.5 12.4 3 17 -15.5 -8 -15.5 8 3 -17 -12.5 -12.4 17 -2.4 z" fill="${TP.creamDeep}" opacity="0.85"/>
  <path d="M1050 640 l6 12 13 1.8 -9.5 9.4 2.3 13 -11.8 -6 -11.8 6 2.3 -13 -9.5 -9.4 13 -1.8 z" fill="${TP.pinkDeep}" opacity="0.6"/>
  <g transform="translate(380 260) scale(5.6)">${TOYS_ART.rabbit}</g>
  <g transform="translate(180 620) scale(2.8)">${TOYS_ART.blocks}</g>
  <g transform="translate(820 560) scale(3)">${TOYS_ART.bear}</g>
</svg>`;
}
...
const TOY_FILES = {
  ...
  'cat-03': [TOYS_ART.blanket, TP.cream, TP.pink], // ของใช้แม่และเด็ก
  'cat-04': [TOYS_ART.onesie, TP.sky, TP.pink], // เสื้อผ้าเด็ก (การ์ดหมวดธีม toys-01)
};
...
await sharp(Buffer.from(toysHero2Svg())).webp({ quality: 85 }).toFile('public/demo/toys/hero-02.webp');
