...
## 2026-07-17 — ธีม "ลิตเติ้ลจอย" toys-01 สำหรับ pack ของเล่น (ref ภาพ Little Joy จากเจ้าของ)
เจ้าของส่งภาพ ref เว็บของเล่นเด็ก "Little Joy" ให้สร้างเป็นเทมเพลทเต็มรูปแบบ — สร้างตามสถาปัตยกรรมธีมเดิม (token + section + variant, grep hex ใน component ใหม่ = 0):
(ก) **ชิ้นใหม่ที่เป็นของกลาง reuse ได้**: ProductCardVariant `toy` (ToyProductCard — การ์ดขาวมน + wishlist + ราคาสี accent + ดาวรีวิวจริง + ปุ่ม "หยิบใส่ตะกร้า" เต็มกว้างเปิด QuickView), ThemeSection `categoryCards` (CategoryCardRow — การ์ดหมวด + ปุ่ม pill พาสเทลสลับ 4 โทน), token อนุพันธ์ใหม่ `--color-accent-soft`/`--color-star-soft` (pattern เดียวกับ primary-soft — pill พาสเทลไม่ต้อง hardcode), `ThemeLayout.utilityBarTone:'soft'` (utility bar พื้น surface แทนพื้น primary), `HeroContent.headline2` (บรรทัดสองสี primary ใน split-panel — ref หัวข้อสองสี น้ำเงิน+ชมพู; เป็น field ใหม่ไม่แตะ headline เดิม = T1 ที่ใช้ split-panel อยู่ไม่กระทบ), TestimonialsBand prop `centered` + **ดาวเปลี่ยน text-primary → text-star** (สเปคเดิมบอกดาวรีวิวสีเดียวทุกธีม — ของเดิมผิดสเปคเฉยๆ ธีมบริการรถได้ดาวทองถูกต้องขึ้นด้วย)
(ข) **preset toys-01 tier 1** — pack ของเล่นตั้ง themeCode นี้ ร้าน signup ด้วย pack นี้ได้หน้าตา ref ทันทีทุกแพลน; utility bar ref มี "ชำระเงินปลายทางได้" → เปลี่ยนเป็นข้อความ PromptPay (ห้ามสัญญา COD — ระบบไม่มี)
