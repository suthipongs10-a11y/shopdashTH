...
- [ ] **Custom domain (บริการ ฿590/ปี — 2026-07-17)**: ลูกค้า *ไม่ต้อง* ตั้ง DNS เองแล้ว — ส่งคำขอ + จ่ายผ่าน PromptPay ที่ `/admin/domain` แล้วทีมงานจัดการให้ทั้งหมด ตรวจ flow ครบกับโดเมนทดสอบ 1 อัน (ดู §โดเมนลูกค้า ด้านล่าง)

## §โดเมนลูกค้า — ขั้นตอนของแอดมินต่อ 1 คำขอ (คิวอยู่ที่ Super Admin → คำขอโดเมน)

1. **ตรวจสลิป** เทียบยอด ฿590 → กด "อนุมัติสลิป — เริ่มดำเนินการ" (ยอดไม่ตรง/โดเมนไม่ว่าง → "ปฏิเสธ" พร้อมเหตุผล ร้านจะเห็นข้อความนั้น)
2. **จดโดเมน** ที่ registrar ของแพลตฟอร์ม (แนะนำใช้บัญชีกลางบัญชีเดียว จดในนามแพลตฟอร์ม) — เช็คความว่างก่อนจด
3. **ตั้ง DNS ของโดเมนนั้น**: CNAME `www` → `cname.shopdashth.com` และ/หรือ A record apex → `76.76.21.21` (ค่าเดียวกับที่ `lib/domains.ts` ใช้ตรวจ)
4. **เพิ่มโดเมนเข้าโปรเจกต์ Vercel** (Domains → Add) เพื่อให้ Vercel ออก TLS cert — ขั้นนี้จำเป็น ไม่งั้นเปิดเว็บไม่ได้แม้ DNS ถูก
5. กด **"ตรวจ DNS"** ในการ์ดคำขอ — ต้องผ่านเช็ค CNAME/A + HTTPS (เช็ค TXT ถูกข้ามอัตโนมัติสำหรับโดเมน managed)
6. กด **"ทำเสร็จ — เปิดใช้โดเมน"** — ระบบสร้าง/ต่ออายุแถว `custom_domains` (active + managed + อายุบริการ 1 ปี) middleware route ให้ทันที
7. ต่ออายุ: ร้านเห็นปุ่ม "ต่ออายุ ฿590/ปี" เมื่อเหลือ ≤45 วัน → คำขอชนิด "ต่ออายุ" เข้าคิวเดิม → แอดมินต่ออายุกับ registrar แล้วกดทำเสร็จ (ระบบขยาย `service_ends_at` +1 ปี) — รายการใกล้หมดอายุโชว์ท้ายหน้าคิว
...
### §8.1 Uptime monitoring (มี endpoint ในระบบแล้ว — 2026-07-16)

ระบบมี `GET /api/health` (public, ไม่ต้อง auth) เช็ค Supabase DB + Auth + R2 จริง แล้วตอบ:
- ทุกอย่างปกติ → HTTP 200 `{"ok":true,"services":{"database":"ok","auth":"ok","r2":"ok"}}`
- service ใดล่ม → HTTP 503 (monitor ทั่วไปตัดสินจาก status code ได้เลย)
- ผล cache ฝั่ง server 30 วินาที + rate limit 20 ครั้ง/นาที/IP — โดนยิงถี่ไม่สร้างโหลดจริง
- รายละเอียด (latency, error message, config ที่ยังไม่ตั้ง) ดูได้ที่ **Super Admin → แดชบอร์ด → สถานะระบบ**

ตั้ง monitor ภายนอก (แนะนำ UptimeRobot ฟรี 50 monitors — uptimerobot.com):
- [ ] Monitor 1: `https://shopdashth.com/api/health` (keyword: `"ok":true` หรือเช็ค HTTP 200) ทุก 5 นาที
- [ ] Monitor 2: `https://{ร้านเดโม่}.shopdashth.com/` ทุก 5 นาที — **สำคัญ**: เส้นทางนี้ผ่าน
      Cloudflare Worker proxy ซึ่ง monitor 1 ไม่ครอบคลุม (Worker พัง = ทุกร้านล่มแต่ /api/health ยังเขียว)
- [ ] ตั้ง alert เข้าอีเมล/LINE Notify ของ UptimeRobot

### §8.2 Error tracking (ยังไม่ได้ติดตั้ง — optional แต่แนะนำ)

- [ ] สมัคร Sentry (ฟรี tier พอ) → `npx @sentry/wizard@latest -i nextjs` → ตั้ง `SENTRY_DSN` บน Vercel
      จับ error 5xx ฝั่ง server + client พร้อม stack trace — ระหว่างยังไม่ติดตั้ง ใช้ Vercel logs ไปก่อน
...
- [ ] ทบทวน rate limit / abuse ของ endpoint public — **มี rate limit ในโค้ดแล้ว (2026-07-10, `lib/rate-limit.ts`)**: checkout 10/นาที, slips 6/นาที, signup 5/ชม., slug-check 30/นาที, discount-check 20/นาที (ต่อ IP) + domain-verify 10/10นาที (ต่อร้าน) — **อัปเดต 2026-07-16: รองรับ Upstash Redis แล้ว** ตั้ง `UPSTASH_REDIS_REST_URL` + `UPSTASH_REDIS_REST_TOKEN` บน Vercel (สมัครฟรีที่ upstash.com → สร้าง Redis database → copy REST credentials) → ตัวเลขนับรวมทุก instance ถูกต้อง; ไม่ตั้ง = นับ in-memory ต่อ instance แบบเดิม (Redis ล่ม = fail-open ไป in-memory อัตโนมัติ ไม่บล็อกลูกค้าจริง) — ถ้าโดนโจมตีจริงจังยังควรเปิด Vercel WAF / edge rate limit เป็นชั้นนอก
